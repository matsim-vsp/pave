...
public class RemovalHeuristics {
	...
	public void shawRemoval() {
		...
		for (RouteElement currentElement : includedRequests) {
			currentElement.setRankingValue(((Settings.shawDistance * (matrix.getTravelTime(selectedElement, currentElement)-minTravelTime)/(maxTravelTime-minTravelTime))) +
					(Settings.shawTime * ((Math.abs(selectedElement.getServiceBegin() - currentElement.getServiceBegin())-minServiceTime)/(maxServiceTime-minServiceTime))));
		}
